<template>
<van-popup :value="isShow" position="bottom" get-container="body" class="co-edit-region-picker" :close-on-click-overlay="false" @click-overlay="hideRegionFn">
...
export default {
    ...
    //     isShow(o,x){
    //         //if(o){
    //             !this.reglist.id ? this.onAddressInfoLoaded() : this.onEdtressInfoLoaded()
    //        // }
    //     } 
    // },
    created(){
    ...
    methods:{
        ...
        resgionsAPI(params){
            // 地址接口
            return axios.get(`${apiHost}/getCity`,{params})
        },
        onEdtressInfoLoaded(){
            // 编辑地址
            let that = this;
            console.log(that.reglist)
            ...
            //     that.citys = regions[province_id];
            //     that.districts = regions[market_id];
            //     return ;
            // }
        ...
        onAddressInfoLoaded() {
            // 新增地址
            let that = this;
            let reglist = that.reglist;
            
            if(!reglist.province_id){
