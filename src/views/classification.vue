<template>
    <div class="tx-dx-classification">
        ...
        <van-tree-select class="tree-select-n2" :items="listch" :main-active-index.sync="active" @click-nav="jsToNav">
            <template #content>
                <div class="select-n2-box" v-for="oc in listObj[active]" :key="oc.id">
                    ...
                    <van-grid :column-num="3" :border="false">
                        <van-grid-item v-for="(ob, obindex) in oc.children" :key="obindex" @click="jsToCommodity(ob)">
...
export default {
  ...
  methods:{
        ...
        firstAPI(type, index){
            ...
            axios.post(`${apiHost}/class`,qs.stringify({type:+type}))
            .then((res) => {
                toast1.clear();
                let {data} = res;
