...
<template>
    <div class="tx-dx-orders">
        ...
        <div class="ordertab">
            ...
            />
            <div>
                <span @click="jsTabsbar(item.id)" :class="[+step === item.id ? 'on':'']" v-for="(item, index) in stepArr" :key="index">{{item.text}}</span>
            </div>
        </div>
        ...
        >
            <ul v-if="list.length>0" class="orderlist">
                <li v-for="(it, index) in list" :key="it.order_id" @click="orderdetailCl(it)">
                    <div class="hed">
                        <span></span>
                        <span class="hed-2">
                            <template v-if="it.cancel === 1">
                                交易已取消
                            </template>
                            <template v-else>
                                {{it.step === 1 ? "交易完成" : it.step === 2 ? "待支付" : it.step === 3 ? "待发货" : it.step === 4 ? "待收货" : "待评论"}}
                            </template>
                        </span>
                    </div>
                    <div class="conbox">
                        <div class="item1">
                            <div class="item1_1" v-for="(op, index) in it.shop.slice(0,4)" :key="index">
                    ...
                    <div class="money">
                        ...
                        <span class="item2">实付：<span>¥ {{it.pay_money}}</span></span>
                    </div>
                    <div class="btnbox">
                        <div v-if="it.step === 1">
                            <!-- <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 98)" v-if="it.cancel == 1">再次购买</van-button> -->
                            <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 97)" v-if="it.cancel == 1">删除订单</van-button>
                        </div>
                        <!-- <div v-if="it.step === 2"> -->
                        <div v-if="it.step !== 1">
                            <!-- <van-button color="#e02e24" @click.stop="redCallFn(it,100)" v-if="it.cancel === 0">去支付</van-button> -->
                            <van-button color="#e02e24" @click.stop="redCallFn(it,100)" v-if="it.cancel === 0 && it.step === 2">去支付</van-button>
                            <!-- <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 98)" v-if="it.cancel == 1">再次购买</van-button> -->
                            <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 99,index)" v-if="it.cancel === 0">取消订单</van-button>
                            <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 97)" v-if="it.cancel == 1">删除订单</van-button>
                        </div>
                        <div v-if="it.step === 3">
                        ...
                        <div v-if="it.step === 4 && it.cancel === 0">
                            <van-button color="#e02e24" @click.stop="redCallFn(it,93,index)">确认收货</van-button>
                            <!-- <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 96)">查看物流</van-button>
                            <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 95)">申请退款</van-button>
                            <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 94)">延长收货</van-button> -->
                        </div>
                        <div v-if="it.step === 5">
                            <van-button color="#e02e24" @click.stop="redCallFn(it,92,index)">立即评价</van-button>
                            <!-- <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 98)">再次购买</van-button> -->
                            <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 97)">删除订单</van-button>
                            <!-- <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 96)">查看物流</van-button>
                            <van-button color="#151516" class="bc9c9c9c" :hairline="true" plain @click.stop="redCallFn(it, 95)">申请退款</van-button>-->
                        </div>
                    </div>
            ...
            <template #finished>
                <div class="orders-empty-1" v-if="finished && +step !==0">没找到订单？试试<span @click="orderAllCl">查看全部</span></div>
                <div class="orders-empty-1" v-if="finished && +step ===0">您已经没有更多的订单了</div>
            </template>
        </van-list>
...
export default {
    data(){
        return{
            list:[],
            listempty:false,
            stepArr: [{id:0,text:"全部"},{id:1,text:"已完成"},{id:2,text:"待支付"},{id:3,text:"待发货"},{id:4,text:"待收货"},{id:5,text:"待评价"}],
            loading:false,
            finished:false,
            page:0,
            limit:8,
    ...
    methods:{
        ...
        redCallFn(o, n, index){
            ...
            if(n === 99){
                that.$dialog.confirm({
                    ...
                    axios.post(`${apiHost}/order/cancel`,qs.stringify({order_id:o.order_id,step:o.step}))
                        .then((res)=>{
                            toast1.clear();
                            let {data} = res;
                ...
                that.$dialog.confirm({
                    ...
                    axios.post(`${apiHost}/order/delete`,qs.stringify({order_id:o.order_id}))
                        .then((res)=>{
                            toast1.clear();
                            let {data} = res;
                            let {status,message} = data;
                ...
                that.$dialog.confirm({
                    ...
                    axios.post(`${apiHost}/order/take`,qs.stringify({order_id:o.order_id}))
                        .then((res)=>{
                            toast1.clear();
                            let {data} = res;
                            let {status,message} = data;
                ...
                const toast1 = that.$toast.loading({
                    ...
                    axios.get(`${apiHost}/order/pay`,{params:{order_id:o.order_id}})
                        .then((res)=>{
                            toast1.clear();
                            let {data} = res;
        ...
        firstAPI(){
            ...
            axios.post(`${apiHost}/order`,qs.stringify({step:+that.step,page:that.page,limit:10})
            ,{
