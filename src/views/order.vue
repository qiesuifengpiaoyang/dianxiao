  ...
  <div class="tx-dx-order">
    ...
    <div class="order-list-2">
        ...
        <van-cell icon="location" class="sad" v-if="list.address">
          ...
          <template #title>
            <div>
              ...
              <p>
                {{ list.address.province }} {{ list.address.market }}
                {{ list.address.area }}
              </p>
      ...
      <div class="money">
        <span></span>
        <span class="item2"
          >实付：<span>¥ {{ list.pay_money }}</span></span
        >
      </div>
    </div>
    ...
    <!-- <div class="clearorder" @click="clearorder" v-if="list.step === 2">
      取消订单
    </div> -->
    <van-submit-bar
      :price="list.pay_money * 100"
      button-text="去支付"
      @submit="onSubmit"
      v-if="+list.step === 2 && !cancel"
    />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
        ...
        .then(() => {
          ...
          axios
            .post(
              ...
              qs.stringify({ order_id: that.order_id, step: that.list.step })
            )
            .then((res) => {
              toast1.clear();
              let { data } = res;
    ...
    onSubmit() {
      ...
      axios
        .get(`${apiHost}/order/pay`, { params: { order_id: that.order_id } })
        .then((res) => {
          toast1.clear();
          let { data } = res;
